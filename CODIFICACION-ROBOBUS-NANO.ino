...
// Contador educativo
int obstaculosDetectados = 0;   // NUEVO ✔
...
void setup() {
  ...
  // Sonido musical de inicio (bonito para los niños 😊)
  melodiaInicio();
...
void loop() {
  ...
  // Revisamos si el celular envió un comando
  if (bluetooth.available() > 0) {
    procesarBluetooth();
  }

  // Si el robot está en modo automático...
  ...
  // Actualizamos luces según el estado
  actualizarLuces();
...
void verificarObstaculo() {
  ...
  if (obstaculoCercano && !enPeligro && !ejecutandoManiobra) {
    ...
    obstaculosDetectados++;    // NUEVO ✔✔✔
    bluetooth.println("Obstáculos detectados: " + String(obstaculosDetectados));

    bluetooth.println("¡OBSTÁCULO! Dist: " + String(distanciaActual));
    ...
    enPeligro = false;
    bluetooth.println("Vía libre");
  }
}

// ------------------------------------------------------------
//        MANIOBRA AUTOMÁTICA DE GIRO
// ------------------------------------------------------------
